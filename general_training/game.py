...
class CatanGame :
    ...
    def generateInputTensor(self, player) : 
        ...
        # create input tensor
        player.input_tensor = torch.zeros([256, 6], device=self.device)
        ...
        for settlement in self.board.settlements : 
            ...
            if case == -1 : 
                ...
                if settlement in settled_player.settlements :
                    player.input_tensor[58+relative_pos][(case - player.number) % n + 1] = 1

                elif settlement in settled_player.cities : 
                    player.input_tensor[58+relative_pos][(case - player.number) % n + 1] = 2
        ...
        for road in self.board.roads : 
            ...
            if case == -1: 
                ...
                player.input_tensor[112+relative_pos][(case - player.number) % n + 1] = 1
        ...
        for update_player in self.players : 
            player.input_tensor[184][(update_player.number - player.number) % n] = update_player.numberOfDevelopmentCards()

        # Players scores (185)
        for update_player in self.players : 
            player.input_tensor[185][(update_player.number - player.number) % n] = update_player.score()

        # Has largest army (186)
        for update_player in self.players : 
            if update_player.has_largest_army : 
                player.input_tensor[186][(update_player.number - player.number) % n] = 1

        # Army sizes (187)
        for update_player in self.players : 
            player.input_tensor[187][(update_player.number - player.number) % n] = update_player.army_size

        # Has longest road (188)
        for update_player in self.players : 
            if update_player.has_longest_road : 
                player.input_tensor[188][(update_player.number - player.number) % n] = 1 

        # Road lengths (189)
        for update_player in self.players : 
            player.input_tensor[189][(update_player.number - player.number) % n] = update_player.roadLength() 

        # Trade ratios (190–194)
        for update_player in self.players : 
            player.input_tensor[190][(update_player.number - player.number) % n] = update_player.trade_ratios["Wheat"]
            player.input_tensor[191][(update_player.number - player.number) % n] = update_player.trade_ratios["Wood"]
            player.input_tensor[192][(update_player.number - player.number) % n] = update_player.trade_ratios["Sheep"]
            player.input_tensor[193][(update_player.number - player.number) % n] = update_player.trade_ratios["Brick"]
            player.input_tensor[194][(update_player.number - player.number) % n] = update_player.trade_ratios["Stone"]

        # Resources per roll (195–249)
        for update_player in self.players : 
            for roll in range(2, 13) : 
                a = (roll-2) * 5 
                player.input_tensor[195+a][(update_player.number - player.number) % n] = update_player.resources_per_roll[roll].count("Wheat")
                player.input_tensor[196+a][(update_player.number - player.number) % n] = update_player.resources_per_roll[roll].count("Wood")
                player.input_tensor[197+a][(update_player.number - player.number) % n] = update_player.resources_per_roll[roll].count("Sheep")
                player.input_tensor[198+a][(update_player.number - player.number) % n] = update_player.resources_per_roll[roll].count("Brick")
                player.input_tensor[199+a][(update_player.number - player.number) % n] = update_player.resources_per_roll[roll].count("Stone")

        # Robber per roll (250)
        resource, number = self.board.tiles[self.board.robber]
        for update_player in self.players : 
            if resource != "Desert" : 
                player.input_tensor[250][(update_player.number - player.number) % n] = len(update_player.robber_per_roll[number])
            else : 
                player.input_tensor[250][(update_player.number - player.number) % n] = 0
        ...
        # Total resources (252)
        for update_player in self.players : 
            player.input_tensor[252][(update_player.number - player.number) % n] = update_player.numberOfResources()
        
        # Buildings (253-255)
        for update_player in self.players : 
            player.input_tensor[253][(update_player.number - player.number) % n] = update_player.buildings["Settlements"]
            player.input_tensor[254][(update_player.number - player.number) % n] = update_player.buildings["Cities"]
            player.input_tensor[255][(update_player.number - player.number) % n] = update_player.buildings["Roads"]
    ...
    def makeMove(self, player) :
        ...
        for i in range(246) : 
            if player.moves_tensor[i] == 1 :
                temp_moves_tensor = torch.zeros(246, device=self.device)
                ...
                # randomly choose this move in order to explore sample space
                if self.has_MCTS == False : 
                    if randint(0, 999) == 0 :
                        ...
                        # we have to clear the previous predictions as they have been changed by a random move
                        for update_player in self.players : 
                            update_player.predictions = []

                        self.has_MCTS = True 

                        break
                
                # if not randomly chosen, decide if the new move is best
    ...
    def runGame(self) : 
        ...
        while max([player.score() for player in self.players]) < 10 and self.turn_number < 250 :
            self.turn_number += 1
        ...
        for player in self.players : 
            if len(player.predictions) > 0 :
                player.predictions = torch.stack(player.predictions)

                player_place = scores.index(player.score())
                player_labels = torch.clone(player.predictions.detach())

                n = len(player_labels)
                for i in range(n-1) : player_labels[i] = player_labels[i+1]
                player_labels[n-1] = player_place
                
                if labels == None : 
                    labels = player_labels
                else : 
                    labels = torch.cat((labels, player_labels))

                if predictions == None : 
                    predictions = player.predictions
                else : 
                    predictions = torch.cat((predictions, player.predictions))

        return predictions, labels, self.turn_number, scores